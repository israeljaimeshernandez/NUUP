...
function setupCheckout() {
  $('#pay-now').addEventListener('click', () => {
    ...
    const summary = `Genera un checkout en ${provider} por ${formatCurrency(total)}. Referencia ${reference}. Cliente: ${email || 'no especificado'}. Notas: ${notes || 'N/A'}`;
  ...
  $('#shipping-quote').addEventListener('click', () => {
    ...
    $('#shipping-summary').textContent = `Cotiza en ${provider.toUpperCase()} aprox. ${formatCurrency(base + extra)} para ${cart.length} productos.`;
  });
}
