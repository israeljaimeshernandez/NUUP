 ...
 *
 * Sistema de encolamiento persistente para solicitudes de alta/baja de
 dispositivos
 * que sobrevive a reinicios del ESP32, garantizando que las operaciones
 pendientes
 * se completen incluso sin conectividad WiFi/MQTT temporal.
 *
 * TIPOS DE DISPOSITIVOS:
 * - Tipo 0: Monitor (este dispositivo) - Alta: alta/0/solicitud
 * - Tipo 1: Sensor NUUP01 Tanque - Alta: alta/1/solicitud | Baja:
 baja/1/solicitud
 * - Tipo 2: [Reservado futuro] - Alta: alta/2/solicitud | Baja:
 baja/2/solicitud
 * - Tipo 3: [Reservado futuro] - Alta: alta/3/solicitud | Baja:
 baja/3/solicitud
 *
...
/*
 * Monitor01 - Versión Optimizada
 * Versión: 86 - Portal AP con botón y resumen de estado
...
//  Definir direcciones para nombre y email (después de tus otras
//  configuraciones)
...
#define BAJAS_PENDIENTES_ADDR                                                  \
  3800                          // Lista persistente de MACs en baja pendiente
...
const char *RESUMEN_CONSECUTIVO =
    "Portal AP solo con botón y resumen inicial de estado";

// Configuración WiFi
#define AP_SSID                                                                \
  "NUUP_monitor01" // que permita el acceso directo finalmente no puede hacer
                   // nada hasta no ingresar un ID de usuario correcto "nuup"
#define AP_PASS ""
#define WIFI_TIMEOUT 5000 // 30 segundos
#define USER_ID_MAX_LEN                                                        \
  32 // Máximo 32 caracteres para el ID lo puedo cambiar si solo necesito el
     // users.users_id concatenado a la clave NUUP2025

// Constantes del sistema
#define MAX_DISPOSITIVOS 50
#define MAC_LEN 17
#define VALORES_POR_DISPOSITIVO 5
#define CONFIG_VERSION 0xA5

// Constantes de tiempo (ms)
#define MQTT_RETRY_INTERVAL_MS 10000
#define MQTT_CONFIRM_TIMEOUT_MS 30000
#define ALTA_PENDING_CHECK_MS 300000
#define BAJA_CONFIRM_TIMEOUT_MS 30000
#define BAJA_PENDING_RETRY_MS 300000
#define PORTAL_SCAN_CACHE_MS 15000
#define DEBUG_DEVICES_INTERVAL_MS 30000
#define DISPLAY_UPDATE_INTERVAL_MS 3000
#define RECONNECT_WIFI_INTERVAL_MS 120000
#define TIEMPO_SIN_DATOS_MS 300000
#define FACTORY_WIFI_TIMEOUT_MS 120000
#define FACTORY_WIFI_RETRY_MS 5000
#define FACTORY_BAJA_TIMEOUT_MS 60000
...
// para asignar un numero de serial al dispositivo lector este viene del LORA
// pero ahorita lo fijmos aqui
...
const unsigned long TIEMPO_SIN_DATOS =
    300000; // 60 000 1 minuto (configurable) - puse 5 minutos
...
String macMonitorFija =
    ""; // MAC del monitor fijada al inicio para evitar usar MAC de sensores
bool macMonitorFijaAnunciada =
    false; // evita spam en consola cuando se dibuja la animación WiFi
...
bool solicitudAltaInicialEnviada =
    false; // Controla que el alta tipo 0 solo se envíe una vez
unsigned long lastConfirmationAttempt = 0;
const unsigned long confirmationTimeout =
    30000; // 30 segundos para esperar confirmación
const unsigned long confirmationRetryInterval =
    10000; // segundos entre reintentos de conexion MQTT para el monitor
const unsigned long altaPendienteInterval =
    5UL * 60UL * 1000UL; // 5 minutos entre solicitudes de alta pendientes
const unsigned long bajaConfirmTimeout =
    30000; // Tiempo de espera de confirmación de baja MQTT
const unsigned long bajaPendienteInterval =
    5UL * 60UL * 1000UL; // Reenvío de bajas pendientes
...
struct UserProfile {
  ...
  char password[32];
  bool flagRegistrado;
  bool bajaMonitorEsperando;
  unsigned long inicioBajaMonitor;
  char ultimaMacBaja[18];
  bool registroEEPROM;
} user = {0};

// Macros de compatibilidad (mantener código existente funcionando)
#define userID (String(user.id))
#define userEmail (String(user.email))
#define userNombre (String(user.nombre))
#define userTelefono (String(user.telefono))
#define userPassword (String(user.password))
#define userFlagRegistrado user.flagRegistrado
#define bajaMonitorEsperandoConfirmacion user.bajaMonitorEsperando
#define inicioEsperaBajaMonitor user.inicioBajaMonitor
#define ultimaMacMonitorBaja (String(user.ultimaMacBaja))
#define registroMonitorEEPROM user.registroEEPROM
// === Alcances ajustables ===
// BLE monitor: POTENCIA_BLE_MONITOR controla la cercanía (N12 ≈ ~5 cm). Sube el
// nivel para más distancia. Proximidad BLE: RSSI_MIN_APAREAMIENTO_MONITOR
// limita el emparejamiento a RSSI igual o mayor. AP monitor:
// alcanceWiFiAPMetrosMonitor define la cobertura objetivo en metros y ajusta
// potenciaTxWiFiAPMonitor automáticamente.
esp_power_level_t POTENCIA_BLE_MONITOR =
    ESP_PWR_LVL_N12; // Ajusta alcance BLE (eleva para más distancia)
int RSSI_MIN_APAREAMIENTO_MONITOR =
    -45; // dBm objetivo (~5 cm) si se habilita proximidad por RSSI
int alcanceWiFiAPMetrosMonitor =
    5; // Alcance estimado del AP en metros (ajustable)
int potenciaTxWiFiAPMonitor =
    11; // dBm aplicados al AP según el alcance deseado
wifi_power_t potenciaWiFiAPMonitor =
    WIFI_POWER_11dBm; // Potencia WiFi usada al iniciar el AP
...
bool portalEnUso =
    false; // Se activa al servir la página para congelar la animación
bool portalPantallaFija = false; // Evita reescribir la pantalla en cada loop
bool reinicioSolicitado =
    false; // Permite reiniciar tras finalizar configuración
unsigned long reinicioProgramado = 0;
// intento de reconectar
unsigned long lastReconnectAttempt = 0;
const unsigned long reconnectInterval = 2 * 60 * 1000;
; // 5 minutos
...
void actualizarDatosDesdeLoRa(const String &mac, const String &mensaje,
                              const String &nombre);
...
// Datos de ejemplo
Dispositivo dispositivos[] = {{"Solares 1", 80, 3.3, 1500},
...
const unsigned char PROGMEM wifiIcon[] = {
    0b00000011, 0b11000000, 0b00001100, 0b00110000, 0b00110000, 0b00001100,
    0b01000000, 0b00000010, 0b00000000, 0b00000000, 0b00000011, 0b11000000,
    0b00001100, 0b00110000, 0b00000000, 0b00000000, 0b00000000, 0b00000000,
...
const unsigned char PROGMEM batteryFull[] = {
    0b00111111, 0b11111100, 0b01100000, 0b00000110, 0b11000000, 0b00000011,
    0b10011111, 0b11111001, 0b10011111, 0b11111001, 0b10011111, 0b11111001,
    0b10011111, 0b11111001, 0b10011111, 0b11111001, 0b10011111, 0b11111001,
    0b10011111, 0b11111001, 0b10011111, 0b11111001, 0b10011111, 0b11111001,
    0b10011111, 0b11111001, 0b11000000, 0b00000011, 0b01100000, 0b00000110,
    0b00111111, 0b11111100};

const unsigned char PROGMEM batteryEmpty[] = {
    0b00111111, 0b11111100, 0b01100000, 0b00000110, 0b11000000, 0b00000011,
    0b10000000, 0b00000001, 0b10000000, 0b00000001, 0b10000000, 0b00000001,
    0b10000000, 0b00000001, 0b10000000, 0b00000001, 0b10000000, 0b00000001,
    0b10000000, 0b00000001, 0b10000000, 0b00000001, 0b10000000, 0b00000001,
    0b10000000, 0b00000001, 0b11000000, 0b00000011, 0b01100000, 0b00000110,
    0b00111111, 0b11111100};
...
void mostrarMensajeFactory(const String &l1, const String &l2 = "",
                           const String &l3 = "");
...
const unsigned long FACTORY_BAJA_TIMEOUT =
    60000; // 1 minuto esperando confirmación
...
// Variables para mostrar detalles de la operación del ALTA o BAJA de un
// dispositivo
...
bool procesarRegistroBLE(const String &macCliente, const String &nombre = "",
                         int altura = 0, int litros = 0) {
  ...
  for (int i = 0; i < MAX_DISPOSITIVOS; i++) {
    if (String(configDispositivos[i].mac) == "") {
      ...
      Serial.println("📏 Altura guardada: " +
                     String(configDispositivos[i].alturaConfig));
      Serial.println("💧 Litros guardados: " +
                     String(configDispositivos[i].litrosConfig));
      Serial.println("📊 Porcentaje inicial: " +
...
bool estaCerca() {
  ...
  Serial.printf("✅ Verificación de proximidad por potencia BLE reducida "
                "(objetivo >= %d dBm si se activa RSSI)\n",
...
private:
  ...
  void handleRegistration(BLECharacteristic *pCharacteristic,
    ...
    Serial.println(
        "⏳ Esperando datos de configuración (CONFIG,nombre,altura,litros)...");
  }

  void handleConfigWithData(BLECharacteristic *pCharacteristic,
                            const String &command) {
    ...
    if (pos1 != -1 && pos2 != -1) {
      ...
      Serial.printf(
          "🎯 Datos parseados - Nombre: '%s', Altura: '%s', Litros: '%s'\n",
          nombre.c_str(), alturaStr.c_str(), litrosStr.c_str());

      // Validar datos
      if (nombre.length() == 0 || alturaStr.length() == 0 ||
          litrosStr.length() == 0) {
      ...
      bool registroExitoso =
          procesarRegistroBLE(ultimaMacCliente, nombre, altura, litros);
  ...
  void handleUnregistration(BLECharacteristic *pCharacteristic,
                            const String &command) {
...
void iniciarBLE() {
  ...
  BLEDevice::setPower(
      POTENCIA_BLE_MONITOR); // Ajusta POTENCIA_BLE_MONITOR para acercar/alejar
  Serial.printf("📡 Potencia BLE del monitor: nivel %d (incrementa para más "
                "alcance, reduce para más cercanía)\n",
  ...
  for (int i = 0; i < MAX_DISPOSITIVOS; i++) {
    if (String(configDispositivos[i].mac) != "") {
      Serial.printf("✅ [%d] MAC: '%s' (longitud: %d)\n", i,
                    ...
                    strlen(configDispositivos[i].mac));
    } else {
...
void testWiFiConnection() {
  ...
  String testPasswords[] = {"JNWWM7KHzE", // Reemplaza con la real
                            "INFINITUM0EBB", "admin", "password",
                            "12345678",      ""};
...
void manejarBotonWifi() {
  ...
  if (wifiButtonPressed && (millis() - wifiButtonPressStart >= TIEMPO_BOTON)) {
    ...
    if (forceAPMode) {
      Serial.println("Presionando boton WiFi - cancelando portal y regresando "
                     "a modo normal...");
...
void configurarAlcanceWiFiAPMonitor(int metros) {
  ...
  Serial.printf("📶 AP Monitor01 - Alcance estimado: %d m | Potencia TX: %d "
                "dBm. Ajusta alcanceWiFiAPMetrosMonitor para cambiarlo.\n",
                alcanceWiFiAPMetrosMonitor, potenciaTxWiFiAPMonitor);
}
...
void startAPMode() {
  ...
  // Atender inmediatamente las primeras peticiones del portal (probes) para
  // evitar timeouts
...
void mostrarMensajeRedConectada(const String &ssid, bool conectado,
                                const String &password, unsigned long retrasoMs,
...
void procesarEscaneoRedes() {
  ...
  if (n <= 0) {
    scannedNetworksCache =
        "<p>No se encontraron redes cercanas. Intenta nuevamente.</p>";
    WiFi.scanDelete();
    return;
  }
  ...
  for (int i = 0; i < limitedCount; i++) {
    ...
    scannedNetworks +=
        "<button class='use-button' data-ssid='" + safeSsidAttr +
        "' type='button' onclick=\"prefillNetwork(this.dataset.ssid, "
        "this)\">Usar</button>";
    scannedNetworks += "</div>";
  }
...
String getCheckedStatus(bool active) { return active ? " checked" : ""; }
...
void handleRoot() {
  ...
  // Mantener el formulario vacío al cargar la página; se llenará al elegir
  // “Modificar” o “Usar”
  String selectedSsid = "";
  String selectedPass = "";
  ...
  for (int i = 0; i < MAX_NETWORKS; i++) {
    if (savedNetworks[i].ssid.length() > 0) {
      ssidOptions += "<option value='" +
                     escapeForHTMLAttr(savedNetworks[i].ssid) + "'></option>";
    }
  }
  ...
  for (int i = 0; i < MAX_NETWORKS; i++) {
    if (savedNetworks[i].ssid.length() > 0) {
      ...
      networksList +=
          "<div class='network-info'><strong>" + safeSsidAttr +
          "</strong><div class='password-label'>Contraseña: " + safePassAttr +
          "</div></div>";
      networksList += "<div class='network-actions'>";
      networksList += "<button class='use-button' data-ssid='" + safeSsidAttr +
                      "' data-pass='" + safePassAttr +
                      "' type='button' onclick=\"editNetwork(" + String(i) +
                      ", this)\">Modificar</button>";
      networksList += "<button type='button' onclick='deleteNetwork(" +
                      String(i) + ")'>Borrar</button>";
      networksList += "</div>";
      networksList += "</div>";
    }
  }
  ...
  for (int i = 0; i < MAX_DISPOSITIVOS; i++) {
    ...
    if (mac.length() > 0) {
      ...
      devicesList += "<div class='device-info'><strong>" + nombre +
                     "</strong><br><small>MAC: " + mac + "</small></div>";
      devicesList += "<label class='device-select'><input type='checkbox' "
                     "name='baja_mac' value='" +
                     escapeForHTMLAttr(mac) +
                     "'> Seleccionar para dar de baja</label>";
      devicesList += "</div>";
    }
  }
  ...
  String correoAttr = userEmail.length() > 0
                          ? " value='" + escapeForHTMLAttr(userEmail) + "'"
                          : "";
  String nombreAttr = userNombre.length() > 0
                          ? " value='" + escapeForHTMLAttr(userNombre) + "'"
                          : "";
  String telefonoAttr = userTelefono.length() > 0
                            ? " value='" + escapeForHTMLAttr(userTelefono) + "'"
                            : "";
  ...
  if (usuarioConfirmado) {
    ...
    String newUserFieldsStyle =
        userFlagRegistrado ? " style='display:none;'" : "";
    String extraDisabled = userFlagRegistrado ? " disabled" : "";
    String extraRequired = userFlagRegistrado ? "" : " required";
    userSection +=
        "<label class='combo-label'><input type='checkbox' id='userRegistered' "
        "name='user_registered' value='1'" +
        checked +
        " onchange=\"toggleUserFields()\"> Ya estoy registrado</label>";
    userSection += "<label for='correoInput'>Correo</label>";
    userSection += "<input id='correoInput' type='email' name='correo' "
                   "placeholder='correo@ejemplo.com' required" +
                   correoAttr + ">";
    userSection += "<div id='newUserFields' class='user-extra-fields'" +
                   newUserFieldsStyle + ">";
    userSection += "  <label for='nombreInput'>Nombre</label>";
    userSection += "  <input id='nombreInput' type='text' name='nombre' "
                   "placeholder='Nombre completo'" +
                   nombreAttr + extraDisabled + extraRequired + ">";
    userSection += "  <label for='telefonoInput'>Teléfono</label>";
    userSection += "  <input id='telefonoInput' type='tel' name='telefono' "
                   "placeholder='Teléfono'" +
                   telefonoAttr + extraDisabled + extraRequired + ">";
    userSection += "  <label for='passUserInput'>Password</label>";
    userSection += "  <input id='passUserInput' type='password' "
                   "name='pass_usuario' placeholder='Contraseña'" +
                   passAttr + extraDisabled + extraRequired + ">";
    userSection += "</div>";
    userSection += "<hr>";
  }
  userSection += "</div>";
  ...
  actionsSection += "  <div class='action-text'><div "
                    "class='icon-badge'>♻️</div><div><strong>Reseteo de "
                    "fábrica</strong><br><small>Solicita la baja, borra la "
                    "EEPROM y reinicia.</small></div></div>";
  actionsSection += "  <button type='button' class='danger-btn' "
                    "onclick=\"factoryReset()\">Reseteo de fábrica</button>";
  actionsSection += "</div>";
  actionsSection += "</div>";
...
<body>
  ...
  html +=
      "<script>const usuarioConfirmadoPortal = " +
      String(usuarioConfirmado ? "true" : "false") +
      "; document.addEventListener('DOMContentLoaded', () => { const "
      "initialSsid = \"" +
      selectedSsidJs +
      "\"; updateSelected(initialSsid, false); const ssidInput = "
      "document.getElementById('ssidInput'); if (ssidInput) { "
      "ssidInput.addEventListener('input', handleManualSsidInput); "
      "ssidInput.addEventListener('focus', handleManualSsidInput); } const "
      "userToggle = document.getElementById('userRegistered'); if (userToggle) "
      "{ userToggle.addEventListener('change', toggleUserFields); } const "
      "configForm = document.getElementById('configForm'); if (configForm) { "
      "configForm.addEventListener('submit', validateUserForm); } "
  ...
  html +=
      "    <input id='ssidInput' type='text' list='ssidOptions' name='ssid' "
      "placeholder='Nombre de la red (SSID)' value='" +
      selectedSsidEscaped +
      "' oninput='handleManualSsidInput()' onfocus='handleManualSsidInput()'>";
  html += ssidOptions;
  html += "    <label for='passInput'>Contraseña</label>";
  html += "    <input id='passInput' type='text' name='pass' "
          "placeholder='Contraseña (visible para editar)' value='" +
          selectedPassEscaped + "'>";
  html += R"=====(
    <div class="network-list">
      <h3 class="section-title">Dispositivos registrados:</h3>
  )=====";
  html += "<p><strong>Selecciona los que desees dar de baja y guarda "
...
void handleSaveCredentials() {
  if (userID.isEmpty() && userEmail.isEmpty()) {
    server.send(400, "text/plain",
                "Configura el usuario (correo) antes de guardar redes");
    return;
  }
  if (server.hasArg("ssid") && server.hasArg("pass")) {
    ...
    // Usar índice solicitado para editar, sobrescribir duplicados o buscar
    // espacio libre
    int indexToSave = (requestedIndex >= 0 && requestedIndex < MAX_NETWORKS)
                          ? requestedIndex
                          : -1;
    if (indexToSave == -1) {
      for (int i = 0; i < MAX_NETWORKS; i++) {
    ...
    server.send(
        200, "text/html",
        "<html><body><h2>Credenciales guardadas. El equipo reiniciará para "
        "aplicarlas.</h2><script>setTimeout(()=>window.location='/',600);</"
        "script></body></html>");
    ...
    reinicioProgramado =
        millis() + retrasoMensajeConexion + duracionMensajeConexion;
  } else {
...
void handleFinalizeConfig() {
  ...
  for (int i = 0; i < server.args() && totalBajasMarcadas < MAX_DISPOSITIVOS;
       i++) {
  ...
  String telefono =
      server.hasArg("telefono") ? server.arg("telefono") : userTelefono;
  String passUsuario =
      server.hasArg("pass_usuario") ? server.arg("pass_usuario") : userPassword;
  ...
  if (!usuarioConfirmado) {
    ...
    if (!banderaRegistrado) {
      if (nombre.length() == 0 || telefono.length() == 0 ||
          passUsuario.length() == 0) {
        server.send(
            400, "text/plain",
            "Completa nombre, teléfono y password para dar de alta al usuario");
        liberarPortal();
        return;
      }
    }
  ...
  if (tieneDatosSsid || tieneDatosPass) {
    if (!tieneDatosSsid || !tieneDatosPass) {
      server.send(400, "text/plain",
                  "Debes capturar SSID y contraseña para guardar la red");
      liberarPortal();
      return;
    }
  ...
  if (totalBajasMarcadas > 0) {
    Serial.printf("🗑️ Baja solicitada desde portal para %d dispositivo(s). Se "
                  "aplicará el mismo flujo que BLE antes del reinicio.\n",
                  totalBajasMarcadas);
    ...
    if (apMode || forceAPMode) {
      Serial.println(
          "🚪 Cerrando modo AP para completar las bajas vía WiFi/MQTT y "
          "retomar el ciclo normal antes del reinicio...");
  ...
  server.send(200, "text/html",
              "<html><body><h2>Configuración guardada</h2></body></html>");
  ...
  if (cerrarAPTrasBajas) {
    ...
    // Mantener el portal atendiendo mientras esperamos el reinicio para evitar
...
void handleDeleteNetwork() {
  if (userID.isEmpty() && userEmail.isEmpty()) {
    server.send(400, "text/plain",
                "Configura el usuario (correo) antes de borrar redes");
    return;
  }
  if (server.hasArg("index")) {
...
void prepararAnimacionBajaPortal(const String &macNormalizada,
                                 const ConfigDispositivo *config) {
...
bool iniciarBajaPortal(const String &mac) {
  ...
  if (indice >= 0) {
    ...
    Serial.println(
        "⚠️  Baja desde portal AP: MAC no encontrada, se mostrará mensaje de "
        "error pero se cerrará el portal para reanudar el ciclo normal.");
    solicitudBajaBLE = true;
    ultimoNombreDispositivo = "No Registrado";
    ultimosLitros = 0;
    ultimaAltura = 0;
...
void handleDeleteDevice() {
  if (userID.isEmpty() && userEmail.isEmpty()) {
    server.send(400, "text/plain",
                "Configura el usuario (correo) antes de borrar dispositivos");
    return;
  }
  if (server.hasArg("mac")) {
    ...
    if (apMode || forceAPMode) {
      Serial.println("🚪 Cerrando modo AP para completar la baja con WiFi/MQTT "
                     "y animaciones como en BLE...");
      detenerConfiguracionWiFi();
    }
  } else {
...
void handleSelectNetwork() {
  if (userID.isEmpty() && userEmail.isEmpty()) {
    server.send(400, "text/plain",
                "Configura el usuario (correo) antes de seleccionar una red");
    return;
  }
  if (server.hasArg("index")) {
    int index = server.arg("index").toInt();
    if (index >= 0 && index < MAX_NETWORKS &&
        savedNetworks[index].ssid.length() > 0) {
      // Desactivar todas
      for (int i = 0; i < MAX_NETWORKS; i++) {
        savedNetworks[i].active = false;
      }
...
void handleSetID() {
  if (server.hasArg("newid")) {
    ...
    if (newID.length() > 0 && newID.length() <= USER_ID_MAX_LEN) {
      ...
      // Mantener activo el portal y recargar la vista completa con
      // redes/dispositivos
      wifiConfigInProgress = true;
      forceAPMode = true;
      apMode = true;
...
void anunciarConsecutivo() {
  ...
  // El consecutivo solo se documenta en consola para evitar ocupar la pantalla
  // inicial
}
...
void iniciarFlujoFactoryReset() {
  ...
  mostrarMensajeFactory("Conectando el", "dispositivo a WiFi",
                        "esperando red...");
...
void manejarFlujoFactoryReset() {
  ...
  case FACTORY_BUSCANDO_WIFI: {
    ...
    mostrarMensajeFactory("WiFi conectado", "Solicitando baja",
                          "al servidor...");
  ...
  case FACTORY_SOLICITANDO_BAJA: {
    ...
    if (enviado) {
      ...
      mostrarMensajeFactory("Solicitando baja", "al servidor...",
      ...
      mostrarMensajeFactory("No se pudo enviar", "la baja por WiFi",
                            "borra manualmente");
      clearEEPROM();
      reinicioSolicitado = true;
      reinicioProgramado = millis() + 4000;
  ...
  case FACTORY_ESPERANDO_CONFIRMACION: {
    unsigned long restante = (factoryConfirmDeadline > millis())
                                 ? (factoryConfirmDeadline - millis()) / 1000
                                 : 0;
    ...
    if (factoryConfirmDeadline > 0 && millis() > factoryConfirmDeadline) {
      mostrarMensajeFactory("Sin confirmación", "Continuando borrado",
                            "manual obligatorio");
      clearEEPROM();
      reinicioSolicitado = true;
      reinicioProgramado = millis() + 4000;
...
void handleFactoryReset() {
  server.send(200, "text/plain",
              "Reseteo solicitado: se pedirá la baja, se borrarán los datos y "
              "se reiniciará.");
...
void handleBajaMonitor() {
  server.send(410, "text/plain",
              "La baja directa fue deshabilitada. Usa el reseteo de fábrica "
              "para solicitarla.");
}
  ...
  for (int i = 0; i < MAX_NETWORKS; i++) {
    ...
    if (savedNetworks[i].ssid.length() > 0) {
      Serial.printf("Red %d cargada: SSID='%s', Pass=%s, Activa=%d\n", i,
                    savedNetworks[i].ssid.c_str(),
                    savedNetworks[i].password.length() > 0 ? "[oculta]"
                                                           : "vacía",
...
void checkWiFiStatus() {
  ...
  case WL_CONNECT_FAILED:
    Serial.println("WL_CONNECT_FAILED (4) - Contraseña incorrecta");
    break;
  ...
  for (int i = 0; i < MAX_NETWORKS; i++) {
    if (savedNetworks[i].ssid.length() > 0) {
      ...
      if (savedNetworks[i].active) {
        ...
        WiFi.begin(savedNetworks[i].ssid.c_str(),
                   savedNetworks[i].password.c_str());
...
void debugNetworks() {
  ...
  for (int i = 0; i < MAX_NETWORKS; i++) {
    if (savedNetworks[i].ssid.length() > 0) {
      anyNetwork = true;
      Serial.printf(
          "Red %d: SSID='%s', Pass='%s', Activa=%d, PassLen=%d\n", i,
          savedNetworks[i].ssid.c_str(),
          savedNetworks[i].password.c_str(), // ← MUESTRA LA CONTRASEÑA
          savedNetworks[i].active ? 1 : 0, savedNetworks[i].password.length());
    }
  }
  if (!anyNetwork) {
...
bool loadUserIDFromEEPROM() {
  ...
  if (len == 0xFF || len < 0 || len > USER_ID_MAX_LEN) {
    Serial.printf(
        "⚠️  Longitud de UserID inválida (%d). Se limpia y se devuelve false.\n",
        len);
    saveUserIDToEEPROM("");
    return false;
  }
...
void saveUserProfileToEEPROM() {
  if (!EEPROM.begin(EEPROM_SIZE)) {
    Serial.println(
        "❌ No se pudo iniciar EEPROM para guardar perfil de usuario");
    return;
  }
...
void callback(char *topic, byte *payload, unsigned int length) {
  Serial.println(
      "callback MQTT ejecutado recepcion-->"); // al principio de la función
  ...
  // ALTA MONITOR / DISPOSITIVOS Validación estricta para el topic de
  // confirmación
  if (strcmp(topic, "alta/0/confirmacion/") == 0) {
    ...
    String registroConfirmado =
        (indices[3] != -1) ? mensajeRecibido.substring(indices[3] + 1) : "";
    ...
    Serial.println(
        "   Se esperaba: MAC,registrado[,correo,nombre,users_registro_id]");

    if (macConfirmada == miMac && (estadoConfirmacion == "registrado" ||
      ...
      Serial.printf("⚠️ Confirmación de monitor ignorada (MAC/estado no "
                    "coinciden): %s / %s\n",
  ...
  if (strcmp(topic, "baja/0/confirmacion/") == 0) {
    ...
    String macConfirmada =
        normalizarMac(mensajeRecibido.substring(0, primeraComa));
    ...
    if (macEsperada.isEmpty()) {
      Serial.println(
          "⚠️  No hay MAC del monitor para validar la confirmación de baja");
      return;
    }

    if (!macConfirmada.equalsIgnoreCase(macEsperada)) {
      Serial.printf(
          "⚠️  Confirmación de baja ignorada: MAC no coincide (%s vs %s)\n",
    ...
    if (estado == "eliminado" || estado == "baja" || estado == "confirmado") {
      Serial.println(
          "✅ Baja MQTT del monitor confirmada; limpiando y reiniciando");
    ...
    if (estado == "eliminado" || estado == "baja" || estado == "confirmado") {
      if (indice != -1) {
        ...
        if (eliminarDispositivo(macConfirmada)) {
          Serial.printf(
              "✅ Baja confirmada y dispositivo %s eliminado localmente\n",
              macConfirmada.c_str());
        } else {
        ...
        Serial.printf(
            "ℹ️ Confirmación de baja recibida para MAC ya eliminada: %s\n",
            macConfirmada.c_str());
      }
  ...
  if (strcmp(topic, "alta/1/confirmacion/") == 0) {
    ...
    if (esMonitor && estadoConfirmacion == "registrado" && mqttConfirmed &&
        ultimoPayloadConfirmMonitor == mensajeRecibido) {
      Serial.printf(
          "ℹ️ [CONF MONITOR01] Confirmación duplicada ignorada para %s\n",
          macConfirmada.c_str());
      return;
    }
    ...
    if (!esMonitor && estadoConfirmacion == "registrado") {
      ...
      if (indiceTemp >= 0 && configDispositivos[indiceTemp].activo &&
          ultimoPayloadConfirmAlta[indiceTemp] == mensajeRecibido) {
        Serial.printf(
            "ℹ️ [CONF NUUP01] Confirmación duplicada ignorada para %s\n",
            macConfirmada.c_str());
        return;
      }
    }
    ...
    if (macConfirmada == miMac) {
      ...
      int cuartaComa = (terceraComa != -1)
                           ? mensajeRecibido.indexOf(',', terceraComa + 1)
                           : -1;
      if (estadoConfirmacion == "registrado" && segundaComa != -1) {
        ...
        String nombreUsuario =
            (terceraComa != -1 && cuartaComa != -1)
                ? mensajeRecibido.substring(terceraComa + 1, cuartaComa)
                : "";
        String registroId =
            (cuartaComa != -1) ? mensajeRecibido.substring(cuartaComa + 1) : "";
        ...
        Serial.println(
            "CONFIRMACION RECIBIDA - Alta del monitor validada correctamente");
        ...
        Serial.println("Formato de mensaje incorrecto para el monitor. Faltan "
                       "datos de usuario");
      }
      ...
      if (indice >= 0) {
        ...
        Serial.printf("✅ [CONF NUUP01] Alta confirmada para %s -> activar "
                      "dispositivo y permitir LoRa->MQTT\n",
                      macConfirmada.c_str());
      ...
      Serial.printf(
          "ADVERTENCIA - Mensaje no reconocido en alta/1/confirmacion/: '%s'\n",
          message);
...
void reconnect() {
  ...
  char clientId[40];
  snprintf(clientId, sizeof(clientId), "Monitor-%08X",
           (uint32_t)ESP.getEfuseMac());

  if (client.connect(clientId, mqtt_user, mqtt_pass)) {
    Serial.println("Conexión MQTT establecida preparado para el alta de "
                   "dispositivo.............");
    Serial.println("Cliente:" + String(clientId) + " Usuario: " + mqtt_user +
                   " PAssword:" + mqtt_pass);
    ...
    Serial.printf(
        "Error en conexión MQTT pero wiffi conectado --> (estado: %d)\n",
        client.state());
  }
}
  ...
  for (int i = 0; i < MAX_DISPOSITIVOS; i++) {
    if (String(configDispositivos[i].mac) != "") {
      Serial.printf("[%02d] MAC: %s | Nombre: %s | Tipo: %d | Activo: %s\n", i,
                    configDispositivos[i].mac, configDispositivos[i].nombre,
                    configDispositivos[i].tipoDispositivo,
                    configDispositivos[i].activo ? "SI" : "NO");
      Serial.printf(
          "      Litros: %.2f/%.2f | Voltaje: %.2f | Temp: %.2f | Altura: %.2f "
          "| %%: %d\n",
          configDispositivos[i].litrosActuales,
          configDispositivos[i].litrosConfig, configDispositivos[i].voltaje,
          configDispositivos[i].temperatura, configDispositivos[i].alturaConfig,
...
void imprimirEstadoBajasPendientes(const char *origen) {
  ...
  for (int i = 0; i < MAX_DISPOSITIVOS; i++) {
    if (!bajaPendienteMQTT[i])
      continue;
    Serial.printf(
        "   RAM[%02d]: MAC=%s | activo=%s | ultimo_intento=%lu | espera=%lu\n",
        i, configDispositivos[i].mac,
        configDispositivos[i].activo ? "SI" : "NO", ultimaSolicitudBaja[i],
        inicioEsperaBaja[i]);
  }
  ...
  for (int i = 0; i < MAX_BAJAS_PERSISTENTES; i++) {
    ...
    Serial.printf("   EEPROM[%02d]: MAC=%s | ultimo_intento=%lu | espera=%lu\n",
                  i, bajasPendientesMac[i], bajasPendientesUltimoIntento[i],
...
bool registrarDispositivo(const String &mac, byte tipo) {
  ...
  for (int i = 0; i < MAX_DISPOSITIVOS; i++) {
    if (normalizarMac(String(configDispositivos[i].mac)) == macNormalizada) {
      ...
      if (bajaPendienteMQTT[i] ||
          buscarSlotBajaPendiente(macNormalizada) != -1) {
        Serial.println(
            "🔁 Re-alta detectada: se limpian banderas/cola de baja pendiente");
      }
  ...
  for (int i = 0; i < MAX_DISPOSITIVOS; i++) {
    if (strlen(configDispositivos[i].mac) == 0) {
      macNormalizada.toCharArray(configDispositivos[i].mac,
                                 MAC_LEN + 1); // +1 para '\0'
...
void cargarDispositivos() {
  ...
  if (usaMensajesCompletos) {
    ...
    Serial.println("⚠️ Formato previo detectado: se asumirá almacenamiento sin "
                   "mensaje LoRa");
  }
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (strlen(mac) == MAC_LEN && mac[2] == ':' && mac[5] == ':') {
      ...
      EEPROM.get(addr, configDispositivos[i].litrosActuales);
      addr += sizeof(float);
      EEPROM.get(addr, configDispositivos[i].voltaje);
      addr += sizeof(float);
      EEPROM.get(addr, configDispositivos[i].temperatura);
      addr += sizeof(float);
      EEPROM.get(addr, configDispositivos[i].alturaConfig);
      addr += sizeof(float);
      EEPROM.get(addr, configDispositivos[i].litrosConfig);
      addr += sizeof(float);

      Serial.printf(
          "📊 Valores leídos: %.1fL, %.1fV, %.1f°C, %.1fcm, %.1fL\n",
          configDispositivos[i].litrosActuales, configDispositivos[i].voltaje,
          configDispositivos[i].temperatura, configDispositivos[i].alturaConfig,
          configDispositivos[i].litrosConfig);

      // LEER PORCENTAJE (2 bytes)
      configDispositivos[i].porcentaje =
          (EEPROM.read(addr++) << 8) | EEPROM.read(addr++);
      ...
      Serial.printf("⚡ Activo leído: %d -> %s\n", activoByte,
                    configDispositivos[i].activo ? "true" : "false");
      ...
      if (usaMensajesCompletos) {
        if (addr >= EEPROM_SIZE) {
          ...
          if (addr + mensajeLen <= EEPROM_SIZE) {
            ...
            Serial.println(
                "❌ Longitud de mensaje fuera de rango, se descarta");
  ...
  for (int i = 0; i < MAX_DISPOSITIVOS; i++) {
    if (String(configDispositivos[i].mac) != "") {
      Serial.printf("📍 configDispositivos[%d]: MAC='%s', Nombre='%s', "
                    "Activo=%d, Tipo=%d, Porcentaje=%d%%, Litros=%.1fL\n",
...
void solicitarAltaMonitorMQTT() {
  ...
  if (WiFi.status() == WL_CONNECTED && client.connected()) {
    if (millis() - lastConfirmationAttempt > confirmationRetryInterval) {
      ...
      // Bloquear si, por error, la MAC fija coincide con algún sensor
      // registrado
      for (int i = 0; i < MAX_DISPOSITIVOS; i++) {
        if (strlen(configDispositivos[i].mac) > 0 &&
            macMonitorFija.equalsIgnoreCase(
                String(configDispositivos[i].mac))) {
          Serial.printf("⛔ Alta de monitor cancelada: la MAC fija %s coincide "
                        "con un sensor en índice %d\n",
      ...
      if (!userFlagRegistrado) {
        if (userTelefono.isEmpty() || userNombre.isEmpty() ||
            userPassword.isEmpty()) {
          Serial.println("⚠️ [ALTA MONITOR01] Falta nombre/teléfono/password "
                         "para alta nueva");
          return;
        }
        mensajeCompleto +=
            "," + userTelefono + "," + userNombre + "," + userPassword;
      }

      Serial.println("   Tópico TX: alta/0/solicitud/");
      Serial.println("   Payload TX: " + mensajeCompleto);
      Serial.println("   Espera RX: alta/0/confirmacion/ con "
                     "'MAC,registrado,correo,nombre,users_registro_id'");
      if (client.publish("alta/0/solicitud/", mensajeCompleto.c_str())) {
        ...
        Serial.println(
            "❌ [ALTA MONITOR01] Error al publicar solicitud de alta");
      }
    }
  }
}
...
bool solicitarBajaMonitorMQTT() {
  ...
  if (registroId.isEmpty()) {
    Serial.println("⚠️ [BAJA MONITOR01] Falta userID/users_registro_id para "
                   "baja/0/solicitud");
    return false;
  }

  if (WiFi.status() != WL_CONNECTED || !client.connected()) {
    Serial.println("⚠️ [BAJA MONITOR01] MQTT desconectado; la solicitud de baja "
                   "no se envió");
    return false;
  }
  ...
  Serial.println("   Espera RX: baja/0/confirmacion/ con 'MAC,eliminado' y "
                 "limpieza total");
...
void solicitarAltaNuupMQTT(int indice, const String &mac) {
  ...
  if (!mqttConfirmed) {
    Serial.println("⚠️ No se puede solicitar alta: el monitor aún no está "
                   "confirmado en MQTT");
    return;
  }
  ...
  bool primeraSolicitud =
      !solicitudAltaEnviada[indice] && ultimaSolicitudAlta[indice] == 0;
  if (!primeraSolicitud &&
      (ahora - ultimaSolicitudAlta[indice] < altaPendienteInterval)) {
    Serial.printf(
        "⏳ Alta de %s ya solicitada; se reintentará en %lu ms\n", mac.c_str(),
  ...
  if (tipo == 0) {
    Serial.printf("⏭️  Alta cancelada: tipo de dispositivo en cero para %s "
                  "(corregir registro)\n",
                  mac.c_str());
    return;
  }
  ...
  if (datosActualizados) {
    if (guardarDispositivos()) {
      Serial.printf(
          "💾 Datos de alta normalizados para %s (nombre='%s', tipo=%d)\n",
          mac.c_str(), nombre.c_str(), tipo);
    } else {
      Serial.println("❌ No se pudieron guardar los datos normalizados antes "
                     "del alta MQTT");
    }
  }

  // Payload completo: MAC,UserID,Nombre,Altura,Litros,Tipo (tipo real
  // registrado)
  ...
  String mensaje = mac + "," + registroId + "," + nombre + "," + altura + "," +
                   litros + "," + String(tipo);

  if (solicitudAltaEnviada[indice] &&
      mensaje == ultimoPayloadAltaMQTT[indice]) {
    Serial.printf("⏭️  Alta MQTT ya enviada con el mismo payload para %s, se "
                  "evita duplicado tipo 0\n",
                  mac.c_str());
    return;
  }

  Serial.println(
      "🛰️ [ALTA NUUP01] Solicitud -> MQTT (ruta separada de monitor)");
  Serial.printf("   Tópico TX: alta/1/solicitud/\n");
  Serial.printf("   Payload TX: %s\n", mensaje.c_str());
  Serial.println(
      "   Espera RX: alta/1/confirmacion/ con 'MAC,registrado' para activarlo");
...
void registrarBajaPendientePersistente(const String &mac) {
  String macNormalizada = normalizarMac(mac);
  if (macNormalizada.length() != MAC_LEN)
    return;
  ...
  if (slot == -1) {
    Serial.println(
        "⚠️ No hay espacio para encolar más bajas pendientes en EEPROM");
    return;
  }
  ...
  Serial.printf("🗂️ Baja pendiente encolada (slot %d) para reintento al "
                "conectar MQTT: %s\n",
                slot, bajasPendientesMac[slot]);
...
bool publicarSolicitudBaja(int indice, const String &mac) {
  ...
  if (tipoDispositivo == 0) {
    Serial.println("❌ ERROR: No se permiten bajas de dispositivos tipo 0 "
                   "(monitor) por este flujo");
    Serial.println(
        "   Use solicitarBajaMonitorMQTT() para dar de baja el monitor");
    return false;
  }
  ...
  if (!mqttConfirmed) {
    Serial.println(
        "⚠️ No se puede solicitar baja: el monitor no está confirmado en MQTT");
    return false;
  }
  ...
  Serial.println("   Payload TX: " + mensaje);
  Serial.println("   Espera RX: baja/" + String(tipoDispositivo) +
                 "/confirmacion/ con 'MAC,eliminado'");

  if (client.publish(topicBaja.c_str(), mensaje.c_str())) {
    ultimaSolicitudBaja[indice] = millis();
    inicioEsperaBaja[indice] = (inicioEsperaBaja[indice] == 0)
                                   ? ultimaSolicitudBaja[indice]
                                   : inicioEsperaBaja[indice];
...
bool publicarSolicitudBajaPersistente(const String &mac, int slot) {
  ...
  if (registroId.isEmpty()) {
    Serial.println(
        "⚠️ No se puede solicitar baja pendiente: falta users_registro_id");
    return false;
  }

  if (!mqttConfirmed) {
    Serial.println("⚠️ No se puede solicitar baja pendiente: el monitor no está "
                   "confirmado en MQTT");
    return false;
  }
...
bool iniciarBajaDispositivo(const String &mac, const char *origen) {
  ...
  if (!config->activo) {
    Serial.printf(
        "ℹ️ Dispositivo %s sin alta MQTT previa, se elimina localmente\n",
        macNormalizada.c_str());
  ...
  // Marcar como pendiente de baja y pausar publicaciones mientras enviamos la
  // solicitud
  config->activo = false;
  bajaPendienteMQTT[indice] = true;
  ...
  // Eliminar inmediatamente del arreglo/EEPROM para que no aparezca más como
  // registrado
...
void procesarBajasPendientes() {
  ...
  if (pendientesRam == 0 && pendientesEEPROM == 0)
    return;
  ...
  if (WiFi.status() != WL_CONNECTED) {
    if (ahora - ultimoAvisoBajas > intervaloAviso) {
      Serial.printf("⏸️  Bajas pendientes (RAM:%d/EEPROM:%d) en espera: WiFi "
                    "desconectado\n",
  ...
  if (!client.connected()) {
    if (ahora - ultimoAvisoBajas > intervaloAviso) {
      Serial.printf("⏸️  Bajas pendientes (RAM:%d/EEPROM:%d) en espera: MQTT "
                    "desconectado\n",
                    pendientesRam, pendientesEEPROM);
      ultimoAvisoBajas = ahora;
    }
  ...
  if (!mqttConfirmed) {
    if (ahora - ultimoAvisoBajas > intervaloAviso) {
      Serial.printf("⏸️  Bajas pendientes (RAM:%d/EEPROM:%d) en espera: monitor "
                    "sin confirmar en MQTT\n",
                    pendientesRam, pendientesEEPROM);
      ultimoAvisoBajas = ahora;
    }
    return;
  ...
  if (userID.isEmpty()) {
    if (ahora - ultimoAvisoBajas > intervaloAviso) {
      Serial.printf("⏸️  Bajas pendientes (RAM:%d/EEPROM:%d) en espera: falta "
                    "users_registro_id\n",
                    pendientesRam, pendientesEEPROM);
      ultimoAvisoBajas = ahora;
    }
  ...
  for (int i = 0; i < MAX_DISPOSITIVOS; i++) {
    ...
    if (inicioEsperaBaja[i] != 0 &&
        (ahora - inicioEsperaBaja[i]) < bajaConfirmTimeout) {
      continue;
    }

    if (ultimaSolicitudBaja[i] == 0 ||
        (ahora - ultimaSolicitudBaja[i]) >= bajaPendienteInterval) {
      publicarSolicitudBaja(i,
                            normalizarMac(String(configDispositivos[i].mac)));
    }
  }
  ...
  for (int i = 0; i < MAX_BAJAS_PERSISTENTES; i++) {
    ...
    if (bajasPendientesInicioEspera[i] != 0 &&
        (ahora - bajasPendientesInicioEspera[i]) < bajaConfirmTimeout) {
      continue;
    }

    if (bajasPendientesUltimoIntento[i] == 0 ||
        (ahora - bajasPendientesUltimoIntento[i]) >= bajaPendienteInterval) {
...
void asegurarMacMonitorFija(const char *motivo) {
  if (macMonitorFija.isEmpty()) {
    ...
    macMonitorFijaAnunciada =
        false; // permitir un anuncio fresco cuando se obtenga de cero
  }
...
void procesarAltasPendientes() {
  ...
  unsigned long ahora = millis();
  if (lastAltaPendienteCheck != 0 &&
      (ahora - lastAltaPendienteCheck < altaPendienteInterval))
    return;
...
void iniciarLoRaConReintentos() {
  ...
  while (!LoRa.begin(433E6)) {
    ...
    intentos++;
    if (intentos >= 10) {
      Serial.println("🚨 ERROR CRÍTICO: No se pudo inicializar LoRa después de "
                     "10 intentos");
      return;
    }
    delay(3000);
...
void recepcion_lora() {
  ...
  if (packetSize) {
    ...
    if (firstComma != -1 && secondComma != -1) {
      String mac =
          normalizarMac(received.substring(firstComma + 1, secondComma));
      ...
      if (tipoDispositivo == 0) {
        Serial.println(
            "⏭️  Mensaje LoRa ignorado: tipo de dispositivo 0 (reservado)");
        return;
      }

      if (!mensajeLoRaTieneDatos(received)) {
        Serial.println(
            "⏭️  Mensaje LoRa ignorado: no contiene todos los datos de sensado");
        return;
      }
      ...
      for (int i = 0; i < MAX_DISPOSITIVOS; i++) {
        ...
        if (storedMac == mac) {
          ...
          if (!configDispositivos[i].activo) {
            Serial.println("⏭️  Dispositivo inactivo: no se envía a MQTT ni se "
                           "solicita alta desde LoRa");
      ...
      if (!encontrado) {
        Serial.println("❌ DISPOSITIVO NO REGISTRADO - Intentando registrar "
                       "con estado inactivo");
        if (registrarDispositivo(mac, tipoDispositivo)) {
          Serial.println("✅ Dispositivo registrado automáticamente desde LoRa "
                         "(activo=0)");
          actualizarDatosDesdeLoRa(mac, received, "");
          ...
          if (nuevoIndice >= 0) {
            ...
            Serial.println("ℹ️  Dispositivo aún inactivo: datos recibidos no se "
                           "publicarán hasta activación en MQTT");
            intentarAltaTrasRegistro(nuevoIndice, mac, "LoRa (nuevo)");
          }
        } else {
          Serial.println(
              "❌ No se pudo registrar el dispositivo (sin espacio)");
        }
      }
    } else {
...
void emparejarNuevoDispositivo() { iniciarEmparejamiento(); }
...
void setWifiStatus(bool conectado) { wifiConectado = conectado; }
...
void mostrarConexionWifi() {
  ...
  // Capturar y fijar la MAC del monitor para evitar confundirla con la de un
  // sensor
...
void conectarWifi() { iniciarAnimacionWifi(); }
...
void dibujarHeader() {
  ...
  if (cantidadDispositivos > 0) {
    Dispositivo dispActual =
        obtenerDatosDispositivo(dispositivoActual % cantidadDispositivos);
    if (dispActual.bateria >= 3.0) {
      display.drawBitmap(SCREEN_WIDTH - 20, 2, batteryFull, 16, 16,
                         SSD1306_WHITE);
    } else {
      display.drawBitmap(SCREEN_WIDTH - 20, 2, batteryEmpty, 16, 16,
                         SSD1306_WHITE);
    }
  } else {
...
void actualizarDatosDesdeLoRa(const String &mac, const String &mensaje,
  ...
  for (int i = 0; i < MAX_DISPOSITIVOS; i++) {
    if (String(configDispositivos[i].mac) == mac) {
      ...
      mensajeLoRa = mensaje; // Garantizar que el mensaje más reciente quede
                             // listo para MQTT
      nuevoMensajeLoRa = configDispositivos[i].activo && mqttConfirmed;

      // Actualizar tipo de dispositivo en caso de que cambie en un mensaje
      // futuro
      int primeraComa = mensaje.indexOf(',');
      if (primeraComa > 0) {
        byte tipoNuevo = (byte)mensaje.substring(0, primeraComa).toInt();
        if (tipoNuevo > 0 &&
            tipoNuevo != configDispositivos[i].tipoDispositivo) {
      ...
      if (commaCount >= 6) {
        ...
        if (commaCount >= 7) {
          ...
          if (nombreExtraido.length() > 0 &&
              nombreExtraido != String(configDispositivos[i].nombre)) {
        ...
        if (configDispositivos[i].litrosConfig > 0) {
          float porcentaje = (configDispositivos[i].litrosActuales /
                              configDispositivos[i].litrosConfig) *
                             100;
...
bool guardarDispositivos() {
  ...
  for (int i = 0; i < MAX_DISPOSITIVOS; i++) {
    if (strlen(configDispositivos[i].mac) > 0) {
      ...
      EEPROM.put(addr, configDispositivos[i].litrosActuales);
      addr += sizeof(float);
      EEPROM.put(addr, configDispositivos[i].voltaje);
      addr += sizeof(float);
      EEPROM.put(addr, configDispositivos[i].temperatura);
      addr += sizeof(float);
      EEPROM.put(addr, configDispositivos[i].alturaConfig);
      addr += sizeof(float);
      EEPROM.put(addr, configDispositivos[i].litrosConfig);
      addr += sizeof(float);
      ...
      if (addr + 1 + mensajeLen > EEPROM_SIZE) {
        Serial.println("❌ ERROR: No hay espacio en EEPROM para guardar el "
                       "mensaje LoRa completo");
        EEPROM.end();
        return false;
      }
    ...
    for (int i = 0; i < MAX_DISPOSITIVOS; i++) {
      if (String(configDispositivos[i].mac) != "") {
        ...
        Serial.printf("    ⚡ Activo MQTT: %s | Tipo: %d | %%: %d\n",
                      ...
                      configDispositivos[i].porcentaje);
        Serial.printf(
            "    📨 Solicitud alta enviada: %s | Última solicitud: %lu ms\n",
            solicitudAltaEnviada[i] ? "SI" : "NO", ultimaSolicitudAlta[i]);
        Serial.printf("    🗑️ Baja pendiente: %s | Inicio espera: %lu ms\n",
                      bajaPendienteMQTT[i] ? "SI" : "NO", inicioEsperaBaja[i]);
        Serial.printf("    💧 Litros: %.1f/%.1f | Altura: %.1f | Voltaje: %.2f "
                      "| Temp: %.1f\n",
                      configDispositivos[i].litrosActuales,
                      configDispositivos[i].litrosConfig,
        ...
        Serial.printf("    🛰️ Último mensaje LoRa hace: %lu ms | Guardado: %s\n",
                      desdeLoRa,
                      ultimoMensajeLoRaDispositivo[i].length() > 0
                          ? ultimoMensajeLoRaDispositivo[i].c_str()
                          : "SIN DATOS");
      }
    }
...
void setup() {
  ...
  // 4. Manejo de arreglo de redes WiFi si existe alguna actualmente dada de
  // alta
  ...
  // Si la EEPROM fue limpiada (flag de registro en 0) pero quedó una
  // confirmación previa, reiniciarla
  if (!registroMonitorEEPROM && mqttConfirmed) {
    Serial.println(
        "Estado MQTT: Limpieza detectada, se borra confirmación previa");
    mqttConfirmed = false;
    guardarMQTTConfirmationState(false);
  }
  ...
  if (mqttConfirmed) {
    ...
    if (!solicitudAltaInicialEnviada) {
      solicitudAltaInicialEnviada =
          true; // Mantener sincronizada la bandera de solicitud
      guardarSolicitudAltaInicialState(true);
    }
    activarDispositivosTrasConfirmacion();
  } else {
  ...
  if (solicitudAltaInicialEnviada && !mqttConfirmed) {
    Serial.println("Estado MQTT: Solicitud de alta inicial ya enviada, en "
...
void loop() {
  ...
  // Si el portal está activo, dedicamos el ciclo completo a atenderlo y
  // evitamos reinicios
  bool apActivo = (WiFi.getMode() & WIFI_MODE_AP) || apMode || forceAPMode;
  if (apActivo) {
    dnsServer.processNextRequest();
    // Atender varias peticiones HTTP por ciclo para mantener la página siempre
    // disponible
    for (int i = 0; i < 3; i++) {
      server.handleClient();
    }
    procesarEscaneoRedes();
    // Mantener la página fija: sin reescaneos automáticos que puedan
    // interrumpir la sesión Si el usuario ya abrió la página evitamos reactivar
  ...
  if (solicitudAltaBLE || solicitudBajaBLE) {
    ...
    static int faseAnimacion =
        0; // 0: giro, 1: resultado, 2: mensaje baja, 3: reinicio
    ...
    } else if (faseAnimacion ==
               2) { // Mensaje final de baja (5s con nombre grande)
      if (transcurrido <= 5000) {
        mostrarMensajeBajaFinal();
      } else {
        bool borradoRAM = macBajaEnCurso.isEmpty() ||
                          obtenerIndiceDispositivo(macBajaEnCurso) == -1;
        bool borradoEEPROM =
            macBajaEnCurso.isEmpty() || !macPresenteEnEEPROM(macBajaEnCurso);
        ...
        reinicioProgramado = millis() + 20000;
               ...
               3) { // Aviso de reinicio y esperar reinicio programado
      mostrarMensajeReinicioBaja();
      // Mantener flags hasta reinicio
    }

    // Reforzar reconexión y reenvío de bajas MQTT aun durante animación
    // (incluye bajas desde portal AP)
    if (!apMode && !forceAPMode) {
      if (WiFi.status() != WL_CONNECTED) {
        attemptReconnectToAllNetworks();
      }
      ...
      if (WiFi.status() != WL_CONNECTED) {
        if (!redesDisponibles) {
          ...
          if (millis() - ultimoAvisoSinRedes > 30000) {
            Serial.println(
                "⚠️  No hay redes guardadas; se omite la activación automática "
                "del portal. Usa el botón WiFi para configurarlo.");
          ...
          if (conteoReintentosWiFi >= 3 && !forceAPMode) {
            Serial.println("⚠️  Sin WiFi tras 3 intentos. Usa el botón WiFi "
                           "para abrir el portal de configuración.");
  ...
  if (WiFi.status() == WL_CONNECTED && client.connected() && mqttConfirmed &&
    ...
    if (nuevoMensajeLoRa) {
      ...
      nuevoMensajeLoRa =
          false; // solo publicar una vez el mensaje y esperar a otro nuevo
    }
  }
  ...
  if (!forceAPMode) {
    ...
    if (dispActual.nombre != ultimoNombreMostrado ||
      ...
      // ⭐⭐ SOLO IMPRIMIR CAMBIOS SIGNIFICATIVOS (cada 30 segundos o cuando
      // cambie)
  ...
  if (millis() - lastStateDebug > 15000) {
    ...
    Serial.printf("👤 UserID: %s\n", userID.c_str());
    Serial.printf(
        "🛂 Subscripciones: alta/0/confirmacion/, alta/1/confirmacion/, "
        "baja/1/confirmacion/, %s/command, %s/estatus\n",
    ...
    for (int i = 0; i < MAX_DISPOSITIVOS; i++) {
      ...
      Serial.printf("[%02d] MAC: %s | Nombre: '%s' | ActivoMQTT: %s | "
                    "AltaSolicitada: %s | BajaPendiente: %s\n",
                    i, configDispositivos[i].mac, configDispositivos[i].nombre,
                    configDispositivos[i].activo ? "SI" : "NO",
                    solicitudAltaEnviada[i] ? "SI" : "NO",
                    bajaPendienteMQTT[i] ? "SI" : "NO");
      Serial.printf(
          "     Últimos datos: %.2fL | %.2fV | %.2f°C | AlturaCfg %.2f | "
          "LitrosCfg %.2f | %% %d\n",
          configDispositivos[i].litrosActuales, configDispositivos[i].voltaje,
          configDispositivos[i].temperatura, configDispositivos[i].alturaConfig,
          configDispositivos[i].litrosConfig, configDispositivos[i].porcentaje);
  ...
  if (millis() - ultimaVerificacion > 10000) { // Cada 10 segundos
    ...
    for (int i = 0; i < MAX_DISPOSITIVOS; i++) {
      if (String(configDispositivos[i].mac) != "") {
        if (millis() - ultimaActualizacionLoRa[i] > TIEMPO_SIN_DATOS &&
            !mostrarSinDatos[i]) {
          Serial.println("⚠️  Dispositivo " + String(i) +
                         " sin datos por más de " +
                         String(TIEMPO_SIN_DATOS / 1000) + " segundos");
          mostrarSinDatos[i] = true;
        }
      }
    }
